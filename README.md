...
This app is setup to be auto-deployed on Heroku. Additionally _review apps_ have been enabled which create an isolated testing environment for each PR. For this we have made some customizations to the base project generated by create-react-app:

1. Added a `Procfile` – command to start the server on Heroku.
2. Added a `app.json` – config for Heroku pipeline.
3. Adds a very basic express server in `./server`.
4. Adds `postinstall` and `server` npm tasks.

## CircleCI

We have enabled continuous integration with CircleCI 2.0. You'll notice the status badge above. CircleCI will run the linter and unit test for each PR and every time you merge to the `master` branch. It has been setup as a required check. Therefore, all your PRs will have to successfully pass testing on CircleCI before it can be merged to master.
